...
# NB: this has to be run with the current working directory as the repo root
...
function kfind() {
    local special_vendor_dirs=()

    if [[ -e vendor/k8s.io/apimachinery ]]; then
        special_vendor_dirs=(${special_vendor_dirs[@]-} "./vendor/k8s.io/apimachinery")
    fi
    if [[ -e vendor/k8s.io/apiserver ]]; then
        special_vendor_dirs=(${special_vendor_dirs[@]-} "./vendor/k8s.io/apiserver")
    fi
