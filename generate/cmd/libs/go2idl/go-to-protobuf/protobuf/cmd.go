...
func New() *Generator {
	...
	common := args.GeneratorArgs{
		...
		GoHeaderFilePath: filepath.Join(sourceTree, "k8s.io/repo-infra/verify/boilerplate/boilerplate.go.txt"),
	}
	return &Generator{
		Common:           common,
		OutputBase:       sourceTree,
		// we can't actually guess these
		// VendorOutputBase: filepath.Join(sourceTree, "k8s.io", "$REPO", "vendor"),
		// ProtoImport:      []string{filepath.Join(sourceTree, "k8s.io", "$REPO", "vendor", "github.com", "gogo", "protobuf", "protobuf")},
		ProtoImport:      []string{},
		Packages: "",
...
func (g *Generator) BindFlags(flag *flag.FlagSet) {
	...
	flag.StringVar(&g.VendorOutputBase, "vendor-output-base", g.VendorOutputBase, "Vendor Output base.")
